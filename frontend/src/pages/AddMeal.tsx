...
export const AddMeal = () => {
  ...
  const MacroIndicator = ({ label, value, color, icon: Icon }: { label: string, value: number, color: string, icon: any }) => (
    <div className="flex flex-col items-center flex-1">
      ...
      <span className="text-lg font-black text-gray-900">{value}g</span>
    </div>
  );
  ...
  return (
    <div className="main-wrapper bg-[#F6F7F9]">
      ...
      {/* Sticky Header */}
      <div className="px-6 pt-12 pb-6 flex items-center sticky top-0 z-40 bg-[#F6F7F9]/90 backdrop-blur-sm">
        <button 
          onClick={() => navigate(-1)}
          className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm active:scale-95 transition-all text-gray-700"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <h1 className="text-xl font-bold text-gray-900 ml-4">Adicionar Refeição</h1>
      </div>
      ...
      <div className="px-6">
        ...
        >
          {/* Food Info Card */}
          <div className="bg-white rounded-[32px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-6 text-center">
            <div className="w-20 h-20 bg-[#F6F7F9] rounded-[24px] flex items-center justify-center mx-auto mb-6 text-4xl shadow-inner">
               🍱
            </div>
            <h2 className="text-3xl font-black text-gray-900 mb-2 leading-tight">{food.name}</h2>
            <p className="text-sm font-medium text-gray-400 mb-8 px-4">Based on your selection, we've calculated the nutrients below.</p>
          ...
          <div className="bg-white rounded-[32px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-6">
            <div className="flex justify-between items-center mb-8">
               ...
               <div className="flex items-center space-x-4">
                 <button 
                   ...
                   className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center active:scale-95 transition-all text-gray-700 hover:bg-gray-100"
                 >
                   <Minus className="w-5 h-5" />
                 </button>
                 <div className="text-center min-w-[100px]">
                    <span className="text-4xl font-black text-gray-900 leading-none">{quantity}</span>
                    <span className="text-lg font-bold text-gray-400 ml-1">g</span>
                 </div>
                 <button 
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] p-6 bg-gradient-to-t from-[#F6F7F9] via-[#F6F7F9]/90 to-transparent z-50">
        <div className="bg-white rounded-[32px] p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] border border-gray-100/50 flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Total Calories</span>
