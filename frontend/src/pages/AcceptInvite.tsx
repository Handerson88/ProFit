    ...
    if (error && !user) {
        return (
            <div className="min-h-screen bg-[#F6F7F9] flex items-center justify-center p-4">
                <div className="bg-white p-8 rounded-[24px] shadow-xl max-w-md w-full text-center space-y-4 border border-rose-100">
                    ...
                    <p className="text-gray-500 font-medium pb-4">{error}</p>
                    <button 
    ...
    return (
        <div className="min-h-screen bg-[#F6F7F9] flex items-center justify-center p-4">
            <motion.div 
                ...
                className="bg-white rounded-[32px] shadow-xl overflow-hidden max-w-md w-full"
            >
                        ...
                        <form onSubmit={handleActivate} className="space-y-4">
                            <div className="space-y-2">
                                <div className="relative opacity-60">
                                    ...
                                    <input 
                                        ...
                                        disabled
                                        value={user?.name || ''}
                                        className="w-full pl-12 pr-4 py-4 bg-gray-100 border-none rounded-2xl text-gray-900 font-medium"
                                    />
                                </div>
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2 pt-2">
                                <div className="relative group">
                                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#56AB2F] transition-colors" size={18} />
                                    <input 
                                        type={showPassword ? "text" : "password"}
                                        required
                                        value={password}
                                        onChange={(e) => setPassword(e.target.value)}
                                        className="w-full pl-12 pr-12 py-4 bg-[#F6F7F9] border-none rounded-2xl focus:ring-2 focus:ring-[#A8E063]/20 transition-all font-medium text-gray-900 placeholder:text-gray-400"
                                        placeholder="Crie uma Senha Segura"
                                    />
                                    <button 
                                        type="button"
                                        onClick={() => setShowPassword(!showPassword)}
                                        className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#56AB2F] transition-colors"
                                    >
